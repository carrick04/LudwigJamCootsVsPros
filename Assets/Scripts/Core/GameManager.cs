...
namespace Chess.Game {
	public class GameManager : MonoBehaviour {
        ...
        private void Awake()
        {
            //banter = transform.Find("message").Find("messageText").GetComponent<TMPro.TMP_Text> ();
        }
        ...
        void Update () {
			...
			if (noOfMoves > 5)
            {
				speechScript.AddWriter(banter, "Try again", 0.1f, true);
				noOfMoves = 0;
			}
		...
		void NewGame (PlayerType whitePlayerType, PlayerType blackPlayerType) {
			...
			noOfMoves = 0; 
			speechScript.AddWriter(banter, "That stupid cat will never realise they have mate in 3...", 0.08f, true);		
		}
		...
		void NotifyPlayerToMove () {
			...
			if (gameResult == Result.Playing) {
				...
				if (noOfMoves == 3)
                {
					speechScript.AddWriter(banter, "I spent my entire life preparing for this", 0.08f, true);
				}
			} else {
				Debug.Log ("Game Over");
				if(noOfMoves <= 5)
                {
					speechScript.AddWriter(banter, "All my training against mittens was useless", 0.08f, true);
				}
			}
			noOfMoves++;
		}
		...
		void PrintGameResult (Result result) {
			...
			if (result == Result.Playing) {
				...
				speechScript.AddWriter(banter, "All my training against mittens was useful afterall", 0.08f, true);
				SceneManager.LoadScene("HIKARU WINS");
			}
			else if (result == Result.BlackIsMated)
			{

				resultUI.text = "Checkmate!";
				SceneManager.LoadScene("COOTS WIN v HIKARU");
